...
def _create_result_files(pm_object, path_results):
    ...
    def check_integer_variables():
        ...
        if False:
            # Only for maintenance
            if len(time_depending_variables_df.index) > 0:
                time_depending_variables_df.to_excel(new_result_folder + '/time_series_binaries.xlsx')

    def build_sankey_diagram(a, only_energy=False, specific_commodity='Hydrogen', average_commodities=True,
                             specific_period=0):
        ...
        for component_object in pm_object.get_final_components_objects():
            if component_object.get_component_type() == 'conversion':
                ...
                generated_commodity_nn = pm_object.get_commodity(generated_commodity).get_name()
        ...
        for variable_name in [*all_variables_dict]:

            if variable_name in variable_two_index:

                for commodity in [*all_variables_dict[variable_name]]:
                    ...
                    list_values = all_variables_dict[variable_name][commodity]
            ...
            elif variable_name in variable_three_index:

                for c in [*all_variables_dict[variable_name]]:

                    for commodity in [*all_variables_dict[variable_name][c]]:
                        ...
                        list_values = all_variables_dict[variable_name][c][commodity]
