...
class GurobiOptimizationProblem:
                        ...
                        if t < max(self.time):
                            self.model.addConstr(
                                ...
                                * self.shut_down_start_up_costs[c]
                                - (1 - self.status_off_switch_off[c, cl, t]) * self.bigM[c],
                        ...
                        for com in self.final_commodities:
                            if com == hot_standby_commodity:
                                self.model.addConstr(self.mass_energy_hot_standby_demand[c, hot_standby_commodity, cl, t]
                                                     ...
                                                     + (self.status_standby[c, cl, t] - 1) * self.bigM[c],
    ...
    def attach_objective_function(self):
        ...
        self.model.setObjective(sum(self.investment[c] * (self.annuity_factor_dict[c] + self.fixed_om_dict[c])
                                    for c in self.all_components)
                                ...
                                + sum(self.restart_costs[c, cl, t] for t in self.time for cl in self.clusters
