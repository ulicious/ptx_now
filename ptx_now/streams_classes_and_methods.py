    ...
    def create_widgets_in_frame(self):
        ...
        if self.demand_var.get():
            ...
            if not self.total_demand_var.get():
                ...
                if self.stream_object.get_unit() == 'MWh':
                    unit_new = 'MW'
                elif self.stream_object.get_unit() == 'GWh':
                    unit_new = 'GW'
                elif self.stream_object.get_unit() == 'kWh':
                    unit_new = 'kW'
                else:
                    unit_new = self.stream_object.get_unit() + ' / h'
...
class AdjustStreamWindow:
    ...
    def create_widgets_in_frame(self):
        ...
        def change_total_demand():
            ...
            if not self.total_demand_var.get():
                if self.stream_object.get_unit() == 'MWh':
                    ...
                    unit_new = self.stream_object.get_unit()
                    text_new = 'Demand [' + unit_new + ' / h]'
            else:
                text_new = 'Demand [' + self.stream_object.get_unit() + ']'
        ...
        if not self.total_demand_var.get():
            if self.stream_object.get_unit() == 'MWh':
                ...
                unit = self.stream_object.get_unit()
                text = 'Demand [' + unit + ' / h]'
        else:
            text = 'Demand [' + self.stream_object.get_unit() + ']'
    ...
    def __init__(self, parent, pm_object, stream):
